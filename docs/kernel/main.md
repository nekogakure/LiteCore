...
2. GDTの構築とインストール
3. ウェルカムメッセージの表示
4. カーネルサブシステムの初期化（`kernel_init()`）
5. テストの実行（TEST_TRUEが定義されている場合）
6. メインループ（`kloop()`）に入る
...
1. キーボードのポーリング
2. 割り込みイベントの処理（`interrupt_dispatch_one()`）
3. シェル未起動の場合、キー入力待ち
4. シェル起動後、コマンドの読み取りと実行
5. 何も処理がない場合、CPUを休止（`cpu_halt()`）
...
- `shell_started`: シェルが起動されたかどうかのフラグ
...
- `INIT_MSG`: 定義されている場合、`kernel_init()`を実行
