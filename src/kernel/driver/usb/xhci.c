...
static void xhci_dump_cap_regs(struct xhci_hc *hc) {
	...
	printk("  HCI Version: %x.%02x\n", 
	       hc->hci_version >> 8, hc->hci_version & 0xFF);
...
uint32_t xhci_get_port_status(struct xhci_hc *hc, uint32_t port_num) {
	...
	volatile struct xhci_port_regs *port = 
		&hc->port_regs[port_num - 1];
 ...
 */
static int xhci_probe_pci_device(uint8_t bus, uint8_t device, uint8_t function) {
	...
	/* Port Registersをマップ */
	hc->port_regs = (volatile struct xhci_port_regs *)
		((uintptr_t)hc->op_regs + 0x400);
	...
	for (bus = 0; bus < 256; bus++) {
		for (device = 0; device < 32; device++) {
			for (function = 0; function < 8; function++) {
				/* ベンダーIDをチェック */
				uint16_t vendor_id = pci_read_config_dword(bus, device, function, 0) & 0xFFFF;
				if (vendor_id == 0xFFFF) {
					continue; /* デバイスなし */
				}
				
				/* Class Code取得 */
				uint32_t class_reg = pci_read_config_dword(bus, device, function, 0x08);
				class_code = (class_reg >> 8) & 0xFFFFFF;
				prog_if = class_reg & 0xFF;
				
				/* USB Controller (Class 0x0C03) かつ xHCI (PI 0x30) */
				if ((class_code >> 8) == 0x0C03 && prog_if == 0x30) {
					/* xHCIコントローラ発見 */
					return xhci_probe_pci_device(bus, device, function);
				}
			}
		}
	}
