...
static uint64_t sys_fstat(uint64_t fd, void *buf) {
    if (buf == NULL) return (uint64_t)-1;
    /* Try to set a sensible st_mode at start of struct stat in user buffer.
       This is a best-effort shim for early testing; proper copy_to_user and
       struct layout awareness should be added later. */
    if (fd == 1 || fd == 2) {
        uint32_t *mode_ptr = (uint32_t *)buf;
        *mode_ptr = 0020000; /* S_IFCHR */
        return 0;
    }
    return (uint64_t)-1;
}
