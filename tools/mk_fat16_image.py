#!/usr/bin/env python3
"""
FAT16ファイルシステムイメージを作成するスクリプト

使い方:
    python3 mk_fat16_image.py <output_image> <size_kb> [files_or_dir...]

例:
    python3 mk_fat16_image.py fat16.img 1024 test.txt readme.md
    python3 mk_fat16_image.py fat16.img 2048 test_data/  # ディレクトリ全体をコピー
"""
...
def create_fat16_image(output_path, size_kb, sources):
    """
    FAT16ファイルシステムイメージを作成する
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        img_path = os.path.join(tmpdir, "fat16.img")
        mount_point = os.path.join(tmpdir, "mnt")
        os.makedirs(mount_point)
        
        print(f"Creating FAT16 image: {output_path} ({size_kb}KB)")
        
        # 空のイメージファイルを作成
        with open(img_path, 'wb') as f:
            f.write(b'\x00' * (size_kb * 1024))
        
        # FAT16 ファイルシステムを作成（mkfs.fat を使用）
        print("Creating FAT16 filesystem...")
        result = subprocess.run(
            ['mkfs.fat', '-F', '16', img_path],
            capture_output=True,
            text=True
        )
        
        if result.returncode != 0:
            print(f"Error: mkfs.fat failed: {result.stderr}")
            return False
...
def main():
    if len(sys.argv) < 3:
        print("Usage: mk_fat16_image.py <output_image> <size_kb> [files_or_dir...]")
        print("Example: mk_fat16_image.py fat16.img 1024 test.txt readme.md")
        print("         mk_fat16_image.py fat16.img 2048 test_data/")
        sys.exit(1)
    ...
    if not create_fat16_image(output_path, size_kb, sources):
        sys.exit(1)
